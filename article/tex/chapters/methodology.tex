...
\chapter{求解高维偏微分方程的随机特征方法}\label{ch:methodology}
  ...
  \section{整体思路}\label{sec:overview}
    ...
    本文实验的统一框架见算法~\ref{alg:general}.

    \begin{algorithm}[ht]
      \SetAlgoLined
      \KwData{方程参数, 时间剖分数 $N$, 样本数 $S$, 随机特征宽度 $H$}
      \KwResult{训练得到的 $y_{0}$, $\alpha$, 以及训练与测试误差}
      生成训练样本路径 $\{X_{k}^{(i)}, \Delta W_k^{(i)}\}$\;
      构造训练阶段的随机特征 $\{H_{k}^{(i)}\}$\;
      \eIf{方程为线性}{
        调用算法~\ref{alg:linear} 求解 $y_0$ 和 $\alpha$\;
      }{
        调用算法~\ref{alg:semilinear} 求解 $y_0$ 和 $\alpha$\;
      }
      在训练路径上计算终端残差 RMSE\;
      在独立测试路径上调用算法~\ref{alg:test} 计算测试 RMSE\;
      返回求解参数与误差指标\;
      \caption{统一实验框架}
      \label{alg:general}
    \end{algorithm}
  ...
  \section{线性方程的求解}\label{sec:linear}
    ...
    \end{enumerate}
    线性方程中参数求解的具体流程见算法~\ref{alg:linear}.
    ...
    \begin{algorithm}[ht]
      ...
      \eIf{$S\ge p$}{
        求解原始岭正则化系统
        \begin{equation*}
          \left(\Phi^{\top}\Phi+\lambda_{\mathrm{r}}R^{\top}R\right)\vartheta
          =
          \Phi^{\top}B,
        \end{equation*}
        得到参数 $\theta$\;
        ...
        \begin{equation*}
          \vartheta = \Phi^{\top}
          \left(\Phi\Phi^{\top} + \lambda_{\mathrm{r}} I_S\right)^{-1}B,
        \end{equation*}
        得到参数 $\theta$\;
      }
  ...
  \section{半线性方程的求解}\label{sec:nonlinear}
    ...
    半线性方程的 LM 迭代流程见算法~\ref{alg:semilinear}.
