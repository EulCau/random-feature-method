...
static inline torch::Tensor randn_like_shape(const std::vector<int64_t>& shape, uint64_t seed) {
    auto gen = torch::make_generator<torch::CPUGeneratorImpl>(seed);
    return torch::randn(shape, torch::TensorOptions().dtype(torch::kFloat32), gen);
}
